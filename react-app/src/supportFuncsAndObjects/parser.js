...
export function parseHTML(htmlText) {
  try {
    const startIndex = htmlText.indexOf('id="Russian">Russian</h2>');
    const rusSection = htmlText.slice(startIndex);
    let declIndex;
    if (rusSection.indexOf('id="Declension') !== -1) {
      ...
      throw new Error('Hopsz, ehhez a szóhoz nem tartozik táblázat az angol nyelvű Wikiszótáron.');
    }
